      ...
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent className='shad-sheet h-screen px-3'>
              ...
              {navItems.map((item, i) => (
                <Link key={i} href={item.url} className='lg:w-full'>
                  <li className={cn("mobile-nav-item", pathname === item.url && 'shad-active')}>
                    ...
                    <p className='lg:block'>{item.name}</p>
                  </li>
                </Link>
              ))}
              </ul>
            ...
            <div className="flex flex-col justify-between gap-5 pb-5">
              <div className='flex w-full justify-center'><FileUploader ownerId={ownerId} accountId={accountId} /></div>
                
                <Button type='submit' className='mobile-sign-out-button' onClick={async () => await signOutUser()}>
                  <Image src='/assets/icons/log-out-svgrepo.svg' alt='logo' height={24} width={24} />
                  <p>Logout</p>
                </Button>
            </div>
        </SheetContent>
