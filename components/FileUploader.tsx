...
const FileUploader = ({ ownerId, accountId, className }: Props) => {
  ...
  const onDrop = useCallback( async (acceptedFiles: File[]) => {
    ...
    const uploadPromises = acceptedFiles.map(async (file) => {
      if(file.size > MAX_FILE_SIZE){
        ...
        return toast({
          description: (<p className='body-2 text-white'><span className='font-semibold'>{file.name}
           is too large. Max file size if 50MB.</span></p>),className: "error-test"
        })
      }
  ...
  // removed , isDragActive from props
  const [files, setFiles] = useState<File[]>([]);

  return (
    <div {...getRootProps()} className='cursor-pointer '>
      <input {...getInputProps()} />
      <AnimatedSubscribeButton className="w-16">
          ...
          {files.map((file,i) => {
            ...
            return(
              <li key={`${file.name}-${i}`} className='uploader-preview-item'>
                <div className="absolute end-full flex items-center gap-3">
